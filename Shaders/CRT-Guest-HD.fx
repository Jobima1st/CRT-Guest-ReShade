/*
	...
	CRT - Guest - HD (Copyright (C) 2018-2025 guest(r))

	Incorporates many good ideas and suggestions from Dr. Venom.

	I would also like give thanks to many Libretro forums members for continuous feedbacks, suggestions and using about the shader.
...
float4 ChromaticPS(float4 position:SV_Position,float2 texcoord:TEXCOORD):SV_Target
{
	...
	if (edgemask > 0.05) {
		...
		float mx3 = COMPAT_TEXTURE(CRTHD_S9, pos1 - 4.0*dx).a;
		float mx4 = COMPAT_TEXTURE(CRTHD_S9, pos1 + 4.0*dx).a;
